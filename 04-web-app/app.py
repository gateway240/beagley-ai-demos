...
def capture_by_frames():
    ...
    boxes_idx, classes_idx, scores_idx = (
        (1, 3, 0) if "StatefulPartitionedCall" in outname else (0, 1, 2)
    )
            ...
            if obj_detect:
                ...
                for i in range(len(scores)):
                    if min_conf_threshold < scores[i] <= 1.0:
                        ymin, xmin, ymax, xmax = [
                            int(coord)
                            for coord in (boxes[i] * [resH, resW, resH, resW])
                        ]
                        cv2.rectangle(
                            frame, (xmin, ymin), (xmax, ymax), (10, 255, 0), 2
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--modeldir", required=True, help="Folder the .tflite file is located in"
    )
    parser.add_argument(
        "--graph", default="detect.tflite", help="Name of the .tflite file"
    )
    parser.add_argument(
        "--labels", default="labelmap.txt", help="Name of the labelmap file"
    )
    parser.add_argument(
        "--threshold", default="0.5", help="Minimum confidence threshold"
    )
    parser.add_argument(
        "--resolution", default="1280x720", help="Desired webcam resolution"
    )
